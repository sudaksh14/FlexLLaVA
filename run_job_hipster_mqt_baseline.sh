#!/bin/bash
# MQT-LLaVA baseline at a fixed query count -- HIPSTER.
#
# Derived from run_job_hipster.sh: same staging, same CUDA pin, same env. Only
# the two stage scripts it calls differ. Those run MQT's OWN train loop from
# MQT-LLaVA/, with a PYTHONPATH guard -- our editable install otherwise shadows
# MQT's `llava` and MQT's train.py silently drives OUR model.
#
#   NUM_VISUAL_TOKENS=4 BASELINE_RUN_TAG=4tok sbatch run_job_hipster_mqt_baseline.sh tinyllama
#SBATCH --job-name=mqt_baseline_hipster
#SBATCH -t 150:00:00
#SBATCH --nodes=1
...
#SBATCH --output=./jobs/run_hipster_mqt_%A.out
#SBATCH --export=ALL
...
echo "[mqt-baseline] NUM_VISUAL_TOKENS=${NUM_VISUAL_TOKENS:-4}"
...
# Each stage cd's into MQT-LLaVA/ itself and re-asserts the PYTHONPATH guard,
# because they run as separate processes.
./scripts/v1_5/pretrain_mqt_baseline_hipster.sh "$SLM_KEY"
./scripts/v1_5/finetune_mqt_baseline_hipster.sh "$SLM_KEY"
